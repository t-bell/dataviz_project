...
#call df to print
pets
#call names() to view columns
names(pets)
#unique pet names
pet_names_unique <- pets %>%
                    distinct(animal_s_name, "[A-Za-z]", 
                    .keep_all = TRUE)
#call new set
print(pet_names_unique)
...
pets %>%
  ...
  ## count number within each group
  count(n=n()) %>%
  ggplot(., aes(x = species, y = zip_code)) + 
  ## geom name for line chart
  geom_line() +
...
## Include code here to explore the data 
## and answer the questions in the exercise## Include code here to explore the data
#call df to print
steak
#call names() to view columns
names(steak)
...
levels(steak)
summary(steak)
#how many responded "yes" to 'do you eat steak?'
table(steak$Do.you.eat.steak.)
#responses to how they like their steak prepared
table(steak$How.do.you.like.your.steak.prepared.)
```
...
ggsave(filename = steak_R.png, plot = last.plot(p) path = "/cloud/project/dataviz_project/figures/explanatory_figures", width = 8, height = 4)
```
